...
class LiraLiteAuditor:
    ...
    def run(self):
        ...
        for sample in tqdm(self.state.samples):
            ...
            # 【动态分支】：按 VLM 分数决定是否进入统计审查
            if vlm_score < 0.1:
                # 认为其极其接近真实非成员，直接 Bypass，最终分为 vlm_score
                final_score = vlm_score
                phy = toolkit.extract_features(sample.loss_sequence)  # 仅用于记录画图
            else:
                # 大于等于 0.1，进入第二阶段叠加
